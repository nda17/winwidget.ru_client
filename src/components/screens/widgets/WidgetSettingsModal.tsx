...
const WidgetSettingsModal = ({ widget, onClose, onSaved }: Props) => {
			...
			<div className={styles.modal}>
				...
				<div className={styles.tabContent}>
					{tab === 'main' && (
						<div className={styles.fields}>
							...
							<div className={styles.field}>
								...
								<p className={styles.hint}>
									Цвет фона карточки (фон под колесом). Оставьте пустым для
									стандартного градиента.
								</p>
							</div>
							...
							<div className={styles.field}>
								...
								<div className={styles.colorRow}>
									<input
										...
										onChange={e => setField('centerColor', e.target.value)}
									/>
									<input
										className={styles.input}
									...
									{config.centerColor &&
										config.centerColor !== '#ffffff' && (
											<button
												type="button"
												className={styles.clearColorBtn}
												onClick={() => setField('centerColor', '#ffffff')}
							...
							<div className={styles.field}>
								...
								/>
								<p className={styles.hint}>
									Через сколько секунд виджет откроется автоматически после
									открытия страницы вашего сайта. Оставьте пустым для
									отключения.
								</p>
							</div>
							...
							<div className={styles.field}>
								...
								/>
								<p className={styles.hint}>
									Текст для посетителя после остановки колеса и определения
									выйгрыша.
								</p>
							</div>
							...
							<div className={styles.field}>
								...
								/>
								<p className={styles.hint}>
									Если посетителей повторное откроет скрытый виджет, увидит
									данное уведомление в интерфейсе виджета.
								</p>
							</div>
							...
							<div className={styles.field}>
								...
								/>
								<p className={styles.hint}>
									По умолчанию ссылка ведет на политику нашего сервиса.
							...
							<div className={styles.field}>
								...
								/>
								<p className={styles.hint}>
									0 — крутить можно только единоразово. Любое другое число
									— посетитель сможет крутить снова через указанное кол-во
									дней.
								</p>
							</div>
								...
								{!confirmResetAttempts ? (
									...
									>
										<p
											...
											style={{ color: '#e05a5a' }}
										>
											Все посетители смогут крутить колесо заново. Действие
											необратимо.
										</p>
										<div style={{ display: 'flex', gap: '8px' }}>
											<button
												...
												className={styles.resetAttemptsBtn}
												onClick={() => {
													resetAttemptsMutation.mutate(crypto.randomUUID())
							...
							<div className={styles.field}>
								...
								<p className={styles.hint}>
									...
									виджета — повторная заявка не сохранится и уведомления о
									заявках не будут вам отправлены.
								</p>
							</div>
							<div className={styles.field}>
								{!confirmReset ? (
									...
									>
										<p
											...
											style={{ color: '#e05a5a' }}
										>
											Все настройки виджета будут заменены на стандартные.
					...
					{tab === 'bonuses' && (
						<div className={styles.fields}>
							<p className={styles.hint}>
								Укажите от 2 до 8 бонусов. <b>Вес</b> управляет частотой
								выпадения сектора: чем больше вес, тем чаще он выпадает.
								Все веса суммируются, и каждый сектор получает долю
								пропорционально своему весу. Пример: 3 сектора с весами 1,
								1, 8 — третий выпадет в 80% случаев. Галочка{' '}
								<b>«никогда не выигрывает»</b> показывает сектор на колесе,
								но исключает его из розыгрыша — вес при этом игнорируется.
							</p>
					...
					{tab === 'integrations' && (
						<div className={styles.fields}>
							...
							<div className={styles.field}>
								...
								/>
								<p className={styles.hint}>
									При открытии виджета отправляется событие <b>ip3_open</b>
									, при отправке заявки — <b>ip3_send</b>. Пиксель VK
									должен быть установлен на странице сайта
								</p>
							</div>
							...
							<div className={styles.field}>
								...
								<p className={styles.hint}>
									При открытии виджета отправляется цель <b>ip3_open</b>,
									при отправке заявки — <b>ip3_send</b>. Код Roistat должен
									быть подключён на странице сайта
								</p>
							</div>
						</div>
					)}
